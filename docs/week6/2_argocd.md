...
사용자는 세 개의 Kubernetes CRD로 Argo CD를 조작합니다. `Application`은 배포의 기본 단위이고, `ApplicationSet`은 여러 Application을 동적으로 생성하는 템플릿이며, `AppProject`는 Application의 소스 저장소, 대상 클러스터, 권한 범위를 제한하는 테넌시 경계입니다[^argocd-concepts].
...
`automated` 블록이 없으면 Manual 모드로 동작해 사용자가 UI나 CLI로 직접 sync를 트리거해야 합니다. 블록이 있으면 Git 변경을 감지할 때마다 자동 sync가 실행되고, 두 하위 옵션으로 자동 sync의 적용 범위를 결정합니다.
...
**Phase는 sync의 생명주기 단계를 나눕니다.** 앞 phase의 모든 리소스가 Healthy가 되어야 다음 phase가 시작됩니다. 정상 흐름에서는 PreSync → Sync → PostSync 순서로 진행되고, Sync phase에 실제 워크로드 리소스가 배치됩니다.

| Phase | Role | Example |
|---|---|---|
| PreSync | Sync 이전 준비 단계 | DB 스키마 마이그레이션, 의존 리소스 사전 생성 |
| Sync | Application 리소스 적용 | Deployment, Service, ConfigMap |
| PostSync | Sync 이후 검증 단계 | smoke test, 배포 완료 알림 |

각 phase에서 실행할 작업은 [hook 리소스](https://argo-cd.readthedocs.io/en/stable/user-guide/sync-waves/#hooks)로 선언합니다. hook은 일반 Kubernetes 매니페스트(주로 `Job`)에 `argocd.argoproj.io/hook: <type>` annotation을 붙여 sync 특정 시점에 실행되도록 표시한 리소스이며, Argo CD는 여섯 가지 hook 타입을 지원합니다.

| Hook type | When it runs | Example |
|---|---|---|
| `PreSync` | Sync phase 직전 | DB 스키마 마이그레이션, 의존 리소스 사전 생성 |
| `Sync` | Sync phase와 함께 | Canary 배포, 점진적 rollout 제어 |
| `PostSync` | 모든 리소스가 Healthy 상태가 된 뒤 | smoke test, 배포 완료 알림 |
| `SyncFail` | Sync phase 실패 직후 | 실패 원인 수집, 롤백 트리거 |
| `PostDelete` | Application 삭제 이후 | 외부 리소스 정리, 감사 로그 기록 |
| `Skip` | 실행하지 않음 | 해당 리소스를 적용 대상에서 제외 |

예를 들어 배포 전 DB 스키마 마이그레이션을 실행하려면 `Job`에 hook annotation을 붙여 Git 저장소에 함께 포함시킵니다.
...
!!! info
    부모 Application 삭제 동작은 `resources-finalizer.argocd.argoproj.io` finalizer 유무로 결정됩니다.

    - finalizer가 있으면 부모 삭제 시 자식 Application과 그 리소스까지 연쇄 삭제됩니다(cascading).
    - finalizer가 없으면 부모만 삭제되고 자식 Application과 실제 리소스는 유지됩니다(non-cascading). 점진적 마이그레이션 중 관리 주체만 분리할 때 사용합니다.
...
List와 Cluster generator는 구조가 단순해 학습용으로 적합하고, 파일 추가만으로 Application을 늘리는 SaaS 테넌트 배포에는 Git generator가 적합합니다. Week 6 Lab의 테넌트 티어 배포 시나리오도 Git generator를 기반으로 구성합니다.

## Argo CD vs Flux

GitOps 도구로 Argo CD를 선택할지 Flux를 선택할지는 운영 요건에 따라 갈립니다. [AWS prescriptive guidance](https://docs.aws.amazon.com/prescriptive-guidance/latest/eks-gitops-tools/use-cases.html)의 비교 표에서 선택에 영향을 주는 항목만 추리면 다음과 같습니다.
...
`Non-EKS clusters`
:   EKS 전용이라 온프레미스, EKS Anywhere, GKE/AKS, 직접 구축한 Kubernetes 클러스터를 동시에 관리할 수 없습니다. 하이브리드나 멀티 클라우드 운영에서는 self-managed Argo CD를 선택해야 합니다.
...
Managed 선택이 적합한 경우는 EKS 전용 환경이고 위 제약을 모두 수용할 수 있을 때로 한정됩니다. 이 문서의 이후 내용과 Lab은 온프레미스, 멀티 클라우드, 팀 단위 RBAC 등 일반적인 실무 요구를 가정해 self-managed Argo CD를 기준으로 합니다. Argo CD 주변에는 canary 배포(Argo Rollouts), 이미지 태그 자동 반영(Image Updater), DAG 실행(Argo Workflows), 이벤트 수신(Argo Events)을 담당하는 Argo 프로젝트들이 있는데, 이들은 이어지는 [Argo Ecosystem](3_argo-ecosystem.md)에서 다룹니다. Argo CD로 ACK와 kro가 관리하는 AWS 리소스까지 함께 선언적으로 배포하는 패턴은 [AWS Integration Patterns](4_aws-integration.md)에서 정리합니다.
