...
앞 장의 RBAC은 Pod이 K8s API를 호출할 때 적용되는 권한 모델이었습니다. Pod이 S3, DynamoDB, KMS 등 AWS API를 호출하려면 IAM 자격 증명이 필요합니다. 워커 노드의 EC2 Instance Profile에 권한을 추가해도 동작은 하지만, 같은 노드에 스케줄된 모든 Pod이 그 권한을 공유하므로 최소 권한 원칙에 어긋납니다.

EKS는 이 문제를 두 가지 방식으로 해결합니다. 2019년에 도입된 IRSA는 OIDC Federation을 사용해 K8s ServiceAccount Token을 STS의 `AssumeRoleWithWebIdentity`로 교환합니다. 2023년 re:Invent에 발표된 EKS Pod Identity는 OIDC Federation 경로 없이 EKS Auth API로 동일한 결과를 얻으며, 설정 단계가 더 적습니다. Pod Identity도 내부적으로 ServiceAccount Token을 사용하지만, 토큰의 제출 대상이 STS가 아니라 EKS Auth API라는 점이 두 방식의 차이입니다. 다음 절에서 IRSA와 Pod Identity를 차례로 정리하고 마지막에 비교합니다.
...
- **EKS OIDC Provider** — `https://oidc.eks.<region>.amazonaws.com/id/<id>`. EKS가 클러스터마다 자동으로 호스팅합니다. private key는 7일마다 회전하며 public key는 만료까지 보관됩니다.[^oidc-rotation]
- **IAM OIDC Identity Provider** — IAM에 위 OIDC Provider URL을 한 번 등록합니다. 클러스터 생성 직후 `eksctl utils associate-iam-oidc-provider`로 만들 수 있습니다.
...
- **K8s ServiceAccount** — `eks.amazonaws.com/role-arn` annotation으로 IAM Role ARN을 가리킵니다.
- **Pod Identity Webhook** — 사용자 클러스터에는 `MutatingWebhookConfiguration` 객체만 등록되어 있고, 실제 webhook 서버는 EKS 컨트롤 플레인에서 EKS가 관리합니다. 새로 만들어지는 Pod이 IRSA SA를 사용하면 이 webhook이 Pod spec을 mutate해 환경 변수와 projected volume을 추가합니다.

[^oidc-rotation]: [IAM roles for service accounts](https://docs.aws.amazon.com/eks/latest/userguide/iam-roles-for-service-accounts.html) — IAM, Kubernetes, and OpenID Connect (OIDC) background

webhook 이름에는 "Pod Identity"가 들어 있지만 EKS Pod Identity 기능보다 먼저 IRSA용으로 도입되었습니다. 현재는 같은 webhook이 IRSA와 EKS Pod Identity 두 메커니즘의 환경 변수와 projected volume을 모두 처리합니다. 이름만 보고 Pod Identity 전용으로 분류하지 않습니다.

### Setup and Runtime Flow

IRSA의 동작은 두 단계로 나뉩니다. 클러스터당 한 번만 수행하는 셋업 단계에서는 EKS 클러스터 생성 시 자동으로 호스팅되는 OIDC Provider URL(`https://oidc.eks.<region>.amazonaws.com/id/<id>`)을 IAM에 OIDC Identity Provider로 등록하고(`eksctl utils associate-iam-oidc-provider`), 이 Provider를 신뢰하는 IAM Role을 생성한 뒤, K8s ServiceAccount에 `eks.amazonaws.com/role-arn` annotation으로 Role ARN을 연결합니다.
...
JWT `aud` claim의 동작은 [Background — Claim 해석](0_background.md#bearer-tokens-and-jwt)에서 다뤘습니다. EKS 환경에서는 세 토큰이 서로 다른 audience를 사용하므로 사용처가 구분됩니다.

| Token Type | `audience` | Verified By |
|---|---|---|
| Default SA Token | `https://kubernetes.default.svc` | K8s API 서버 |
...
AWS Load Balancer Controller는 IRSA가 적용된 대표적인 add-on입니다. `eksctl create iamserviceaccount` 한 번으로 IAM Role, Trust Policy, K8s ServiceAccount, annotation을 한꺼번에 생성할 수 있습니다.
...
- **SDK 최소 버전 요구** — IRSA는 SDK가 `AssumeRoleWithWebIdentity`를 지원해야 동작합니다. 대표 버전은 boto3 ≥ 1.9.220, AWS SDK for Java v2 ≥ 2.10.11, AWS SDK for Go v1 ≥ 1.23.13, AWS CLI ≥ 1.16.232입니다. 전체 표는 [공식 문서](https://docs.aws.amazon.com/eks/latest/userguide/iam-roles-for-service-accounts-minimum-sdk.html)를 참조합니다.
- **`hostNetwork: true` Pod의 IMDS 노출** — 호스트 네트워크를 사용하는 Pod은 노드의 IMDS에 항상 접근할 수 있습니다. SDK는 IRSA 자격 증명을 우선 사용하지만, IMDS 자체를 막지는 못한다는 점에 주의합니다.
- **지원 환경** — IRSA는 EKS, EKS Anywhere, ROSA, EC2 자체 관리형 Kubernetes에서 모두 사용할 수 있습니다. EKS on Outposts local cluster에서는 사용 불가입니다.
- **EKS OIDC Provider URL은 public** — 클러스터의 OIDC Provider URL과 JWKS 엔드포인트는 인증 없이 접근할 수 있습니다. EKS 외부 시스템도 K8s ServiceAccount Token을 받아 서명을 검증할 수 있어 사내 인증 게이트웨이나 외부 워크로드 페더레이션에 사용할 수 있습니다.
...
[^pod-id-agent]: [Learn how EKS Pod Identity grants pods access to AWS services — Considerations](https://docs.aws.amazon.com/eks/latest/userguide/pod-identities.html#pod-id-considerations) — "It uses the node's hostNetwork, occupying port 80 and 2703 on the link-local address (169.254.170.23 for IPv4, [fd00:ec2::23] for IPv6)."
...
| Supported environments | EKS, EKS Anywhere, ROSA, EC2 자체 관리 K8s | EKS only |
| Minimum EKS version | 모든 지원 버전 | 1.24+ |
| Pod Identity association limit | 해당 없음 | 5,000 / cluster |
| Introduced | 2019 | 2023 re:Invent |

신규 EKS 클러스터이고 운영 환경이 EKS로 한정된다면 Pod Identity를 우선 검토합니다. 클러스터 간 Role 재사용, 세션 태그 기반 ABAC, 설정 단계 수에서 Pod Identity가 IRSA보다 적은 비용을 요구합니다. 기존 IRSA 자산이 많거나 EKS Anywhere, self-managed K8s 같은 환경에 동일한 인증 메커니즘을 적용해야 하는 경우에는 IRSA를 유지합니다. 두 방식은 같은 클러스터 안에서 공존할 수 있으므로 점진적으로 마이그레이션할 수 있습니다.

---

## Considerations Common to Both

- **컨테이너는 보안 경계가 아닙니다** — IRSA와 Pod Identity는 Pod 단위로 자격 증명을 분리해주지만, 같은 노드의 다른 Pod에서 권한을 탈취당할 가능성을 완전히 제거하지는 않습니다. Pod이 사용하는 IAM Role에는 항상 최소 권한 원칙을 적용합니다.
- **link-local 주소와 포트 차단 여부 확인** — Pod Identity Agent는 `169.254.170.23` (IPv4) 또는 `[fd00:ec2::23]` (IPv6)의 포트 `80`과 `2703`에 바인딩됩니다. 보안 솔루션, NetworkPolicy, 호스트 방화벽이 이 주소나 포트를 차단하면 자격 증명 발급이 실패합니다. IPv6를 사용하지 않는 환경이라면 [Disable IPv6 in the EKS Pod Identity Agent](https://docs.aws.amazon.com/eks/latest/userguide/pod-id-agent-config-ipv6.html) 절차로 IPv6 바인딩을 비활성화해야 Agent가 정상 기동합니다.
- **CloudTrail 이벤트 소스가 다릅니다** — IRSA는 `eventSource: sts.amazonaws.com` / `eventName: AssumeRoleWithWebIdentity`로 기록되고, Pod Identity는 `eventSource: eks-auth.amazonaws.com` / `eventName: AssumeRoleForPodIdentity`로 기록됩니다. 감사 쿼리를 작성할 때 두 소스를 모두 포함해야 합니다.
- **SDK 버전** — Pod Identity는 [별도의 SDK 최소 버전 표](https://docs.aws.amazon.com/eks/latest/userguide/pod-id-minimum-sdk.html)를 따릅니다. IRSA용 SDK와 호환되지 않을 수 있으므로 마이그레이션 전 SDK 업그레이드가 선행되어야 합니다.
