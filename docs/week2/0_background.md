...
K8s에서 Pod은 각자 IP를 하나씩 갖습니다. 이 IP는 클러스터 내부에서 임의로 정한 주소 대역(Pod CIDR)에서 할당됩니다. 예를 들어 `10.244.0.0/16`라 가정합니다. 문제는 이 주소가 **클러스터 외부 네트워크에서는 존재하지 않는다**는 점입니다.

노드들은 실제 네트워크 주소를 갖고 있습니다(예: `192.168.1.10`, `192.168.2.20`). 하지만 `10.244.1.5`라는 Pod IP는 어느 스위치에도, 어느 라우터에도 등록된 적 없는 가상의 주소입니다. 그래서 Node 1의 Pod이 Node 2의 Pod에게 패킷을 보내면, 중간 네트워크는 이 패킷을 어디로 보내야 할지 모릅니다.
...
이게 가능한 건 AWS ENI(Elastic Network Interface) 덕분입니다. EC2 인스턴스는 ENI 하나에 여러 개의 Secondary IP를 붙일 수 있는데, VPC CNI는 이 Secondary IP를 Pod에 직접 할당합니다. Pod이 `192.168.1.20`을 받으면, 그건 실제로 EC2 ENI에 붙은 VPC 주소입니다.
...
CNI는 kubelet이 Pod 네트워크를 설정할 때 사용하는 플러그인 인터페이스 스펙입니다. Pod이 생성되면 kubelet이 `/etc/cni/net.d/` 설정을 읽고 CNI 플러그인 바이너리를 호출합니다.
...
VPC CNI에서 IP를 실시간으로 확보하려면 EC2 API를 호출해야 합니다. 새 ENI를 인스턴스에 연결하는 것은 단순한 메모리 조작이 아니라 EC2 컨트롤 플레인에 요청을 보내고 하이퍼바이저가 가상 NIC를 생성하기까지 기다리는 과정으로, 보통 **5~10초**가 걸립니다. Pod이 생성될 때마다 이 지연이 발생하면 오토스케일링 상황에서 수백 개의 Pod이 줄지어 대기하게 됩니다.
