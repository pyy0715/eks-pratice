...
- 모든 컨테이너는 NAT 없이 다른 모든 컨테이너와 통신할 수 있어야 합니다
- 모든 노드는 NAT 없이 모든 컨테이너와 통신할 수 있어야 합니다 (반대 방향도 마찬가지)
- 컨테이너가 자신의 IP 주소로 인식하는 값은 외부에서 해당 컨테이너를 바라볼 때의 IP 주소와 동일해야 합니다
...
:   노드의 라우팅 테이블에 따라 eth0을 통해 VPC 네트워크로 전달되며, VPC 라우팅에 따라 대상 노드의 ENI로 전달됩니다.
...
각 Pod는 독립된 network namespace를 가지며, 같은 Pod 안의 컨테이너들은 하나의 namespace를 공유합니다. Pod에서 나가는 모든 트래픽은 veth pair를 통해 Host network namespace로 진입합니다.
...
Pod에 할당되는 IP는 `/32` prefix로, 서브넷 마스크가 없어 Pod은 목적지에 상관없이 모든 패킷을 gateway로 전달합니다. VPC CNI는 Pod 스케줄링 시 default route의 gateway로 `169.254.1.1`(RFC 3927 link-local)을 설정합니다. 즉, 스위치나 라우터처럼 실제로 존재하는 게이트웨이가 아니라, AWS VPC CNI가 Pod의 라우팅을 위해 설정해둔 가상의 게이트웨이 주소입니다.

따라서 `169.254.1.1`은 실제 네트워크에 존재하지 않는 주소입니다. Pod이 이 gateway로 패킷을 보내려면 Ethernet 프레임에 다음 Hop MAC 주소를 넣어야 합니다. 
하지만 Pod은 ARP 프로토콜을 사용하여 네트워크에 브로드캐스트로 조회해도 gateway의 주소가 존재하지 않기 때문에 응답하지 않습니다.

이를 위해 VPC CNI는 Pod network namespace에 정적 ARP 항목을 미리 등록해 ARP 브로드캐스트 없이 veth로 곧바로 전달합니다.(`169.254.1.1 → 호스트 veth MAC`)
...
???+ info "CNI setup sequence — veth, routes, ARP, policy rules"
...
노드에는 **Primary ENI**(eth0)와 Pod IP를 추가로 수용하기 위한 **Secondary ENI**(eth1, eth2…)가 있습니다. Secondary ENI는 Primary ENI와 다른 서브넷에 속할 수 있어 gateway IP가 다릅니다. 라우팅 테이블이 하나뿐이라면 문제가 생깁니다. 

예를 들어 Pod IP `10.0.97.30`이 eth1에 속할 때, 이 Pod로 들어오는 패킷은 eth1을 통해 도달합니다. 그러나 라우팅 테이블의 default route는 eth0을 가리키므로 응답 패킷이 eth1 대신 eth0으로 나갑니다. AWS VPC는 각 ENI에 Source/Destination Check를 적용하고 있어, eth0이 자신의 IP가 아닌 소스 주소(Pod IP)를 가진 패킷을 차단합니다.
...
다른 노드에 있는 Pod로 향하는 패킷은 노드 eth0을 나가 VPC 네트워크를 통해 대상 노드 ENI로 전달됩니다.
...
VPC CNI는 iptables SNAT 규칙을 설치해 VPC 외부로 나가는 트래픽의 소스 IP를 Pod IP에서 Primary ENI IP로 변환합니다.
...
!!! tip "Disabling External SNAT"
    ...
    === "Private Subnet + NAT Gateway"
        Pod이 Private Subnet에 있고 NAT Gateway를 통해 인터넷에 나가는 구조라면, 노드에 Public IP가 없으므로 CNI의 SNAT이 의미가 없습니다. `EXTERNALSNAT=true`로 CNI SNAT을 끄고 서브넷 라우팅 테이블의 NAT Gateway가 처리하게 해야 합니다.
