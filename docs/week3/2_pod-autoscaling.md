...
VPA는 Pod의 `resources.requests`를 실제 사용량 기반으로 최적값으로 자동 조정합니다. 추천값은 기준값(최소 필요값)에 마진(버퍼)을 더하여 산출됩니다.

VPA가 추천값을 적용할 때는 기존 Pod를 종료하고 새 Pod를 생성합니다. Kubernetes는 실행 중인 Pod의 `resources.requests`를 직접 변경하지 못합니다. Pod의 리소스는 컨테이너 런타임이 cgroup을 통해 설정하는데, 이미 실행 중인 프로세스의 cgroup 설정을 변경하면 예기치 않은 동작이 발생할 수 있어 Kubernetes가 이를 허용하지 않습니다. Kubernetes 1.27부터 `InPlacePodVerticalScaling` 기능이 알파 상태로 도입되고 있어, 향후 재시작 없이 리소스를 조정하는 방향으로 발전하고 있습니다.

VPA는 Kubernetes 내장 기능이 아니라 별도 add-on으로 설치해야 합니다. VPA는 CRD, MutatingWebhook, 세 개의 컨트롤러를 모두 설치해야 작동하는 복잡한 구성이기 때문에, 모든 클러스터에 기본 탑재하기에는 운영 부담이 큽니다.

### VPA Components

VPA는 세 개의 컴포넌트가 협력하여 동작합니다. 기능을 하나의 프로세스에 합치지 않고 분리한 이유는 각 컴포넌트의 권한과 부작용이 다르기 때문입니다. Recommender는 읽기 전용으로 부작용이 없고, Updater는 Pod를 evict하는 권한을 갖고, Admission Controller는 Pod 생성 경로에 동기적으로 개입합니다. 이 분리 덕분에 `updateMode: Off`로 설정하면 Recommender만 동작해 추천값을 제공하고 실제 적용은 하지 않는 안전한 운영이 가능합니다.

**Recommender**
:   리소스 사용량 히스토리를 모니터링하고 추천값을 계산합니다. metrics-server 또는 Prometheus에서 데이터를 수집합니다. 읽기 전용이므로 클러스터에 부작용이 없습니다.

**Updater**
:   추천값과 현재 Pod의 `resources.requests`를 비교하여 차이가 크면 해당 Pod를 eviction합니다. eviction은 Kubernetes의 정상적인 Pod 종료 메커니즘을 따르므로 PodDisruptionBudget을 준수합니다.
...
세 컴포넌트의 협력 흐름은 Recommender → Updater → Admission Controller 순서로 이루어집니다. Recommender가 추천값을 계산하면, Updater가 현재값과 차이가 클 때 Pod를 evict하고, Admission Controller가 새 Pod 생성 시점에 추천값을 자동 주입합니다.

/// figure-caption
![VPA 컴포넌트 아키텍처 — Recommender, Updater, Admission Controller](assets/vpa-architecture.png)

[Source: AWS Startups](https://www.youtube.com/watch?v=jLuVZX6WQsw)
///
...
!!! warning "HPA + VPA Conflict"
    동일한 메트릭(CPU, Memory)에 HPA와 VPA를 함께 적용하면 충돌이 발생합니다. VPA가 CPU request를 늘리면 HPA는 사용률이 낮아진 것으로 판단해 replica를 줄이고, Pod당 부하가 올라가면 다시 VPA가 CPU request를 늘리는 순환이 반복됩니다.

    - VPA(CPU/Memory) + HPA(Custom/External 메트릭) — 서로 다른 메트릭을 사용하므로 충돌 없음
...
!!! warning "JVM applications with VPA"
    ...
    `-XX:MaxRAMPercentage`를 사용하면 컨테이너 메모리 limits의 비율로 힙 크기를 동적 계산합니다.
...
HPA는 Metrics API(`metrics.k8s.io`)를 통해 수집한 CPU, Memory 메트릭만 트리거로 사용합니다. SQS 큐 길이, Kafka consumer lag, Prometheus 쿼리 결과처럼 외부 이벤트를 스케일링 기준으로 삼으려면 KEDA(Kubernetes Event-Driven Autoscaling)가 필요합니다.

KEDA는 HPA를 대체하지 않고 확장합니다. HPA는 이미 replica 계산 알고리즘, stabilization window, Kubernetes 스케줄러 통합을 갖추고 있습니다. KEDA가 이를 처음부터 재구현하는 대신, `external.metrics.k8s.io` API를 통해 외부 이벤트 데이터를 HPA가 읽을 수 있는 메트릭으로 변환해 노출합니다. HPA 입장에서는 SQS 메시지 수나 Kafka lag도 일반 메트릭과 다를 바 없습니다. ScaledObject를 생성하면 KEDA가 내부적으로 HPA를 자동 생성하고, Kubernetes 스케줄러와의 통합은 HPA가 그대로 담당합니다.
...
`keda-operator` (Agent)
:   ScaledObject를 감시하고 Deployment를 활성화/비활성화합니다. 이벤트가 없으면 replica를 0으로 줄이고, 이벤트가 발생하면 다시 활성화합니다.

`keda-operator-metrics-apiserver` (Metrics)
:   Kubernetes metrics server로 등록되어 외부 이벤트 소스의 데이터를 `external.metrics.k8s.io` API로 노출합니다. HPA는 이 API에서 External 메트릭을 읽어 스케일링 결정에 사용합니다.

`keda-admission-webhooks`
:   ScaledObject 변경 시 misconfiguration을 차단합니다. 예를 들어 동일한 Deployment를 타겟으로 하는 ScaledObject가 여러 개 생성되는 것을 방지합니다.

### ScaledObject

ScaledObject는 스케일링 대상 Deployment와 트리거를 정의하는 KEDA의 핵심 CRD입니다. KEDA는 ScaledObject를 기반으로 HPA를 자동 생성하며, 트리거에는 다양한 외부 이벤트 소스를 지정할 수 있습니다. Cron 탭에서 전체 spec 구조를 확인하고, Kafka와 SQS 탭은 `triggers` 항목만 교체한 예시입니다.

=== "Cron"

    특정 시간대에 replica 수를 미리 늘려두는 proactive 스케일링입니다.
...
=== "Kafka"
    ...
    Kafka consumer lag를 트리거로 사용합니다. `lagThreshold`에 도달하면 replica를 증가시킵니다.
...
=== "SQS"
    ...
    SQS 큐의 메시지 수를 트리거로 사용합니다. EKS Pod Identity로 인증하면 별도 자격증명 없이 SQS에 접근할 수 있습니다.
...
KRR(Kubernetes Resource Recommender)은 Prometheus 데이터를 기반으로 리소스 추천값을 제공하는 CLI 도구입니다(robusta-dev/krr). VPA는 클러스터 내부에서 Pod 이벤트를 실시간으로 감시하고 eviction까지 수행하기 때문에 반드시 클러스터 안에 설치해야 합니다. 반면 KRR은 Prometheus의 HTTP API에 읽기 전용으로 접근해 과거 사용량 데이터를 분석하기만 하므로, 로컬 머신에서 실행해도 됩니다. 클러스터에 아무것도 설치하지 않고 추천값을 먼저 확인한 뒤, 실제 적용 여부를 직접 판단할 수 있다는 점에서 운영 환경에서 안전하게 사용할 수 있습니다.
