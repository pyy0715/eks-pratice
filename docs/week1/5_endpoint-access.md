...
Managed ENI의 경우, 서로 다른 AZ에 2개의 ENI를 고객 VPC 안에 생성합니다. **API Server가 워커 노드의 kubelet에 접근할 때**(kubectl logs, exec 등) 이 ENI를 통해 고객 VPC로 진입합니다.
...
클러스터 생성 시 기본 설정입니다. 사용자의 kubectl과 워커 노드의 kubelet 모두 인터넷을 통해 NLB로 접근합니다. VPC 내부에서 출발한 트래픽도 VPC 밖으로 나가지만 Amazon 네트워크 내에서만 이동합니다.

!!! warning
    워커 노드의 트래픽이 인터넷을 경유하므로 운영 환경에서는 권장하지 않는 구성입니다.
...
![Private](https://d2908q01vomqb2.cloudfront.net/fe2ef495a1152561572949784c16bf23abb28057/2020/04/10/endpoint_private.png)
