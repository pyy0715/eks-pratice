...
5xx의 직접 원인은 Gateway 전환 과정에서 배포 매니페스트를 정리하며 `preStop` sleep과 `terminationGracePeriodSeconds`가 짧아진 데 있었습니다. 이전에는 이 두 값이 ALB deregister 전파 지연을 흡수하고 있었지만, 값이 줄어들자 전파가 끝나기 전에 들어온 연결이 종료 중인 Pod으로 그대로 전달되면서 5xx가 나타났습니다.
...
!!! tip "Lesson"
    ...
    - **PDB는 tradeoff입니다** — `minAvailable: 100%`는 가용성을 보장하지 않으면서 유지보수만 차단합니다. 비계획 disruption(장애, spot 중단)에는 무력하고 계획된 disruption(consolidation, 업그레이드)만 막습니다. PDB는 "어느 수준까지 견딜 수 있는가"에 대한 답이어야 합니다.
    - **검증 가능한 PDB 설정** — `kubectl get pdb -A`에서 `disruptionsAllowed`가 0인 PDB가 있으면 해당 서비스는 모든 노드 유지보수가 불가능합니다. OPA/Kyverno 같은 정책 엔진으로 `minAvailable`이 100%인 PDB 생성을 차단할 수 있습니다.
    - **두 보호 계층** — PDB은 Pod 단위 보호막이고 `disruption.budgets`은 노드 단위 속도 제한입니다. 두 장치가 함께 작동해야 consolidation이 안전하게 진행됩니다.
...
EC2NodeClass의 `role` 필드에 IAM role 이름이 틀려 있었습니다. 마이그레이션 과정에서 CAS가 사용하던 IAM instance profile 이름을 참고해 EC2NodeClass를 작성했으나, role 이름 형식(`KarpenterNodeRole-${CLUSTER_NAME}`)에 오타가 있었습니다. Karpenter는 EC2 `RunInstances` API를 호출할 때 이 IAM role을 instance profile로 지정하는데, 존재하지 않는 role이면 API 호출이 실패합니다. NodeClaim의 `status.conditions`에서 `LaunchFailed`와 함께 에러 메시지를 확인할 수 있었습니다.
